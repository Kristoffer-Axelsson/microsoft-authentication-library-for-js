...
import { XhrClient, XhrResponse } from "../network/XhrClient";
import { UrlUtils } from "../utils/UrlUtils";
import TelemetryManager from "../telemetry/TelemetryManager";
import HttpEvent from "../telemetry/HttpEvent";
import { TrustedAuthority } from "./TrustedAuthority";
import { NetworkRequestType, Constants, WELL_KNOWN_SUFFIX } from "../types/Constants";
...
export class Authority {
    ...
    private DiscoverEndpoints(openIdConfigurationEndpoint: string, telemetryManager: TelemetryManager, correlationId: string): Promise<ITenantDiscoveryResponse> {
        ...
        return client.sendGetRequestAsync(openIdConfigurationEndpoint)
            .then((response: NetworkResponse<any>) => {
                httpEvent.httpResponseStatus = response.status;
